...
# إضافة مجلد src إلى مسار بايثون ليعرف كيف يجد الملفات بداخله
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), 'src')))

# الآن سيجد الملفات لأننا حددنا المسار الصحيح
...
def run_trading_engine():
    ...
    logger.info(f"--- النظام يعمل الآن - المسارات مصححة ---")
        ...
        try:
            ...
            if current_bar_time != last_checked_time:
                ...
                if is_valid and risk_data['is_valid']:
                    if paper_engine.execute_order(symbol, "BUY", price):
